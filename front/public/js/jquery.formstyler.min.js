...
h+'<ul style="position: relative; list-style: none; overflow: auto; overflow-x: hidden">'+w+"</ul></div>");e.append(f);var n=c("ul",f),l=c("li",f),t=c("input",f),x=c("div.jq-selectbox__not-found",f).hide();l.length<d.selectSearchLimit&&t.parent().hide();var u=0,A=0;l.each(function(){var a=c(this);a.css({display:"inline-block"});a.innerWidth()>u&&(u=a.innerWidth(),A=a.width());a.css({display:"block"})});var h=e.clone().appendTo("body").width("auto"),v=h.width();h.remove();v==
...
"-webkit-overflow-scrolling":"touch"}),k=c("li",e).attr("unselectable","on").css({"-webkit-user-select":"none","-moz-user-select":"none","-ms-user-select":"none","-o-user-select":"none","user-select":"none"}),d=a.attr("size"),h=m.outerHeight(),f=k.outerHeight();void 0!==d&&0<d?m.css({height:f*d}):m.css({height:4*f});h>e.height()&&(m.css("overflowY","scroll"),q(m),k.filter(".selected").length&&m.scrollTop(m.scrollTop()+k.filter(".selected").position().top));a.prependTo(e).css({position:"absolute",
...
(function($) {
		$(function() {
			$('.report-request-form div select, .tab-content input').styler();
		})
		})(jQuery)